...
RSpec.describe RailsConsoleAi::Repl do
  ...
  describe 'consecutive execute_code detection' do
    it 'injects warning after CONSECUTIVE_EXEC_WARN consecutive execute_code calls' do
      # LLM calls execute_code with slightly different code each time (puts vs p vs inspect)
      call_count = 0
      allow(mock_provider).to receive(:chat_with_tools) do
        call_count += 1
        if call_count <= 3
          RailsConsoleAi::Providers::ChatResult.new(
            text: '',
            input_tokens: 50, output_tokens: 20,
            tool_calls: [{ id: "tc_#{call_count}", name: 'execute_code', arguments: { 'code' => "puts #{call_count}" } }],
            stop_reason: :tool_use
          )
        else
          chat_result("The answer is 42.")
        end
      end
      allow(mock_provider).to receive(:format_assistant_message).and_return(
        { role: 'assistant', content: [{ 'type' => 'tool_use', 'id' => 'tc_1', 'name' => 'execute_code', 'input' => {} }] }
      )
      allow(mock_provider).to receive(:format_tool_result).and_return(
      ...
      engine = repl.instance_variable_get(:@engine)
      _result, _msgs, _stats = engine.send(:send_query_with_tools, [{ role: :user, content: 'count users' }])

      # Should have called 4 times (3 execute_code rounds + final after warning)
      expect(call_count).to eq(4)
    end

    it 'breaks loop after CONSECUTIVE_EXEC_BREAK consecutive execute_code calls' do
      # LLM keeps calling execute_code endlessly with different code
      call_count = 0
      allow(mock_provider).to receive(:chat_with_tools) do
        call_count += 1
        RailsConsoleAi::Providers::ChatResult.new(
          text: '',
          input_tokens: 50, output_tokens: 20,
          tool_calls: [{ id: "tc_#{call_count}", name: 'execute_code', arguments: { 'code' => "variant_#{call_count}" } }],
          stop_reason: :tool_use
        )
      end
      final_result = chat_result("I gave up.")
      ...
      # Should break at 5 (CONSECUTIVE_EXEC_BREAK), not run forever
      expect(call_count).to eq(5)
    end

    it 'resets consecutive count when a non-execute_code tool is used' do
      call_count = 0
      allow(mock_provider).to receive(:chat_with_tools) do
        call_count += 1
        case call_count
        when 1, 2
          # Two execute_code calls
          RailsConsoleAi::Providers::ChatResult.new(
            text: '',
            input_tokens: 50, output_tokens: 20,
            tool_calls: [{ id: "tc_#{call_count}", name: 'execute_code', arguments: { 'code' => "code_#{call_count}" } }],
            stop_reason: :tool_use
          )
        when 3
          # A different tool (resets counter)
          RailsConsoleAi::Providers::ChatResult.new(
            text: '',
            input_tokens: 50, output_tokens: 20,
            tool_calls: [{ id: "tc_#{call_count}", name: 'list_tables', arguments: {} }],
            stop_reason: :tool_use
          )
        when 4, 5
          # Two more execute_code calls (counter should be 1, 2 — not 3, 4)
          RailsConsoleAi::Providers::ChatResult.new(
            text: '',
            input_tokens: 50, output_tokens: 20,
            tool_calls: [{ id: "tc_#{call_count}", name: 'execute_code', arguments: { 'code' => "code_#{call_count}" } }],
      ...
      # Should reach round 6 (no warning/break triggered because counter reset at round 3)
